...
+ (void)showSuccessWithStatus:(NSString*)status
{
    ...
    _currentCount = [NSNumber numberWithInt:[_currentCount intValue] + 1];
    [KGStatusBar performSelector:@selector(dismiss:) withObject:_currentCount afterDelay:Delay ];
...
+ (void)dismiss : (NSNumber*) count{
    if ([count intValue] == [_currentCount intValue]) {
        [[KGStatusBar sharedView] dismiss];
    }
}
